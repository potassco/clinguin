...
class TkinterGui(AbstractGui):
    ...
    def _window_startup(self, id, parent, attributes, callbacks):
        ...
        tka = {
            "width" : Attribute(id, "width", root.winfo_screenwidth(), "geometry(tka['width'].value + 'x' + tka['height'].value + '+' + str(int(root.winfo_screenwidth()/2 - int(tka['width'].value)/2)) + '+' + str(int(root.winfo_screenheight()/2 - int(tka['height'].value)/2)))"),

            "height" : Attribute(id, "height", root.winfo_screenheight(), "geometry(tka['width'].value + 'x' + tka['height'].value + '+' + str(int(root.winfo_screenwidth()/2 - int(tka['width'].value)/2)) + '+' + str(int(root.winfo_screenheight()/2 - int(tka['height'].value)/2)))"),
            "backgroundcolor" : Attribute(id, "background", "white", "configure(background = tka['backgroundcolor'].value)"),
            "resizablex" : Attribute(id, "resizablex", 1, "resizable(tka['resizablex'].value,tka['resizabley'].value)"),
            "resizabley" : Attribute(id, "resizabley", 1, "resizable(tka['resizablex'].value,tka['resizabley'].value)"),
            }
    ...
    def dropdownmenu(self, id, parent, attributes, callbacks):
        ...
        tka = {
            "width" : Attribute(id, "width", 50, None),
            "backgroundcolor" : Attribute(id, "background", "white", "configure(background = tka['backgroundcolor'].value)"),
            "selected" : Attribute(id, "variable", "", None),
            "initially_selected" : Attribute(id, "variable", "", None),
            }
