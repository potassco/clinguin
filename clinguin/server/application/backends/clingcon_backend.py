...
class ClingconBackend(ClingoBackend):
    """
    ...
    def _init_command_line(self):
        ...
        clingcon_config = (
            [a.split("=") for a in self._args.clingcon_config]
            if self._args.clingcon_config
            else []
        )
        self._clingcon_conf = [(a[0], a[1]) for a in clingcon_config]
    ...
    def _create_ctl(self):
        ...
        for k, v in self._clingcon_conf:
    ...
    def register_options(cls, parser):
        """
        Adds the `clingcon-config` option.
        """
        ...
        parser.add_argument(
            ...
            help=textwrap.dedent(
                """\
                    Clingcon options list of <parameter>=<value>.
                 """
            ),
            nargs="*",
        )
