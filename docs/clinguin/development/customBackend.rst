...
If you are using *clingo*, we highly recommend extending the :ref:`ClingoBackend` to create your own.
This backend contains multiple functionalities already built in which can be overwritten and extended.
...
Each backend can define any number of public operations or override the existing ones.
...
.. warning::
    ...
    - If the operation impacts the current browsing state, make sure to call the :meth:`~ClingoBackend._outdate` method.
    - If the operation makes the current control outdated (for instance, adds an atom or a program), make sure to call the :meth:`~ClingoBackend._init_ctl` method.
...
These methods are involved in how the domain control is solved.
They can be overwritten for things such as theory extensions.
...
.. warning::
    ...
    Some domain constructors will be skipped if they are not used in the ui-files.
...
.. note::
    ...
    Domain state constructors for this backend are shown in the :ref:`ClingoBackend` and :ref:`ClingoBackend` sections.
...
can be made by overriding this method.
