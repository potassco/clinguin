...
This frontend was developed using `Angular <https://angular.io/guide/setup-local>`_. For styling, the `bootstrap <https://getbootstrap.com/>`_ library is used, with the `Angular-Boostrap <https://ng-bootstrap.github.io/#/home>`_ library (which has very cool and beautiful components out of the box). This frontend redirects interactions on the webpage to communicate with the server. It implements most of the elements and attributes of tkinter. This frontend then gives access to Bootstrap classes for styling.
