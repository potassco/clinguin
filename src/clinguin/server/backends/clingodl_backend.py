...
from clinguin.server.backends.clingo_backend import (
    ClingoBackend,
)
...
class ClingoDLBackend(ClingoBackend):
    ...
    def _init_command_line(self):
        ...
        dl_config = [a.split("=") for a in self._args.dl_config] if self._args.dl_config else []
    ...
    def _on_model(self, model):
        ...
        # pylint: disable=attribute-defined-outside-init
        self._assignment = list((key, val) for key, val in self._theory.assignment(model.thread_id))
